...
class Admin_Bar
{
    ...
    {
        require_once BIT_WC_SMTP_PLUGIN_DIR_PATH . '/views/view-root.php';

        /* echo plugin_basename( BIT_WC_SMTP_PLUGIN_MAIN_FILE );
      $query = new WP_Query();
      ...
      var_dump($forms); */
        ...
        wp_enqueue_script('bit_wc_smtp-admin-script', BIT_WC_SMTP_ASSET_URI . '/js/index.js');
        $bit_wc_smtp = apply_filters('bit_wc_smtp_localized_script', array(
            'nonce'           => wp_create_nonce('bit_wc_smtp'),
            'confirm'         => __('Are you sure?', 'bit_wc_smtp'),
            'isPro'           => false,
            'routeComponents' => array('default' => null),
            'mixins'          => array('default' => null),
            'assetsURL'       => BIT_WC_SMTP_ASSET_URI . '/js/',
            'baseURL'         => $base_apth_admin . 'admin.php?page=bit_wc_smtp#/',
            'ajaxURL'         => admin_url('admin-ajax.php')
        ));
        // $checkoutFields = (new WC_Checkout)->get_checkout_fields();
        // $checkoutFields = WC()->checkout()->get_checkout_fields();
        // $bit_wc_smtp['checkoutFields'] = (new WC_Checkout)->get_checkout_fields();
        wp_localize_script('bit_wc_smtp-admin-script', 'bit_wc_smtp', $bit_wc_smtp);
    }
    ...
    public function admin_footer_text($footer_text)
    {
        ...
        $is_bit_wc_smtps_screen = ($current_screen && false !== strpos($current_screen->id, 'bit_wc_smtp'));

        if ($is_bit_wc_smtps_screen) {
            $footer_text = sprintf(
                __('If you like %1$s please leave us a %2$s rating. A huge thank you from %3$s in advance!', 'bit_wc_smtp'),
                '<strong>' . __('bit_wc_smtp', 'bit_wc_smtp') . '</strong>',
                '<a href="https://wordpress.org/support/plugin/bit_wc_smtp/reviews/" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>',
